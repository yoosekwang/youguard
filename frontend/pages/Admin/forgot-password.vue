<template>
    <div class="min-h-screen flex items-center justify-center bg-gray-100">
        <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
            ...
            <h1 class="text-2xl font-bold mb-6 text-center">Forgot Admin Password</h1>
            ...
            <p class="mt-4 text-center">
                <router-link to="/Admin/signin" class="primary-color text-blue-500 hover:underline">{{ $t("forgot_password_back_to_login") }}</router-link>
            </p>
        </div>
    </div>
</template>
...
const submitEmail = async () => {
    ...
    try {
        const response = await axios.post(
            `${useRuntimeConfig().public.apiBase}/api/admin/forgot-password`,
            { email: email.value },
            {
                headers: {
