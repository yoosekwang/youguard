        ...
        <div class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-12">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                 ...
                 <div class="bg-white rounded-lg shadow-lg">
                    <div class="p-6">
                        <div class="flex flex-col md:flex-row items-center gap-6">
                            <div class="relative">
                                <div class="h-32 w-32 rounded-full bg-primary-50 relative overflow-hidden">
                                    <img v-if="developerData.avatar" :src="developerData.avatar"
                                        class="h-full w-full object-cover" alt="Profile Avatar" />
                                    <div v-else
                                        class="h-full w-full flex items-center justify-center bg-indigo-100 text-indigo-600 text-2xl font-bold">
                                        {{ getInitials(developerData.name) }}
                                    </div>
                                </div>
                            </div>
                            <div class="text-center md:text-left">
                                <h1 class="text-2xl font-bold text-gray-900">{{ developerData.name }}</h1>
                                <p class="text-gray-500">{{ developerData.email }}</p>
                                <p class="text-sm text-gray-400 mt-1">{{ developerData.title || 'System Administrator' }}
                                </p>
                ...
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div class="border-b border-gray-200">
                        <nav class="flex -mb-px">
                            <button v-for="tab in devtabs" :key="tab.id" @click="devActiveTab = tab.id" :class="[
                                'flex-1 px-4 py-4 text-center border-b-2 font-medium text-sm',
                                devActiveTab === tab.id
                                    ? 'border-indigo-500 text-indigo-600'
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                    ...
                    <!-- Profile Tab Content -->
                    <div v-if="devActiveTab === 'profile'" class="p-6">
                        <form @submit.prevent="handleDevProfileUpdate" class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Full Name</label>
                                    <input v-model="devProfileForm.name" type="text"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Email</label>
                            ...
                            <div class="flex justify-end">
                                <button type="submit" :disabled="isDevSubmitting"
                                    class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">
                                    <svg v-if="!isDevSubmitting" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" />
                                        <polyline points="17 21 17 13 7 13 7 21" />
                                    ...
                                    {{ isDevSubmitting ? 'Saving...' : 'Save Changes' }}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <!-- Password Tab Content -->
                    <div v-if="devActiveTab === 'security'" class="p-6">
                        <form @submit.prevent="handleDevPasswordChange" class="space-y-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Current Password</label>
                                <div class="relative">
                                    <input :type="showDevPassword ? 'text' : 'password'"
                                        v-model="devPasswordForm.currentPassword"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                    <button type="button" @click="showDevPassword = !showDevPassword"
                                        class="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5">
                                        <svg v-if="!showDevPassword" xmlns="http://www.w3.org/2000/svg" width="1.5em"
                                            height="1.5em" viewBox="0 0 20 20">
                                            <path fill="currentColor"
                                                d="M18.521 1.478a1 1 0 0 0-1.414 0L1.48 17.107a1 1 0 1 0 1.414 1.414L18.52 2.892a1 1 0 0 0 0-1.414zM3.108 13.498l2.56-2.56A4.2 4.2 0 0 1 5.555 10c0-2.379 1.99-4.309 4.445-4.309c.286 0 .564.032.835.082l1.203-1.202A13 13 0 0 0 10 4.401C3.44 4.4 0 9.231 0 10c0 .423 1.057 2.09 3.108 3.497zm13.787-6.993l-2.562 2.56c.069.302.111.613.111.935c0 2.379-1.989 4.307-4.444 4.307c-.284 0-.56-.032-.829-.081l-1.204 1.203c.642.104 1.316.17 2.033.17c6.56 0 10-4.833 10-5.599c0-.424-1.056-2.09-3.105-3.495" />
                                        </svg>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">New Password</label>
                                <div class="relative">
                                    <input :type="showDevNewPassword ? 'text' : 'password'"
                                        v-model="devPasswordForm.newPassword"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                    <button type="button" @click="showDevNewPassword = !showDevNewPassword"
                                        class="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5">
                                        <svg v-if="!showDevNewPassword" xmlns="http://www.w3.org/2000/svg" width="1.5em"
                                            height="1.5em" viewBox="0 0 20 20">
                                            <path fill="currentColor"
                                                d="M18.521 1.478a1 1 0 0 0-1.414 0L1.48 17.107a1 1 0 1 0 1.414 1.414L18.52 2.892a1 1 0 0 0 0-1.414zM3.108 13.498l2.56-2.56A4.2 4.2 0 0 1 5.555 10c0-2.379 1.99-4.309 4.445-4.309c.286 0 .564.032.835.082l1.203-1.202A13 13 0 0 0 10 4.401C3.44 4.4 0 9.231 0 10c0 .423 1.057 2.09 3.108 3.497zm13.787-6.993l-2.562 2.56c.069.302.111.613.111.935c0 2.379-1.989 4.307-4.444 4.307c-.284 0-.56-.032-.829-.081l-1.204 1.203c.642.104 1.316.17 2.033.17c6.56 0 10-4.833 10-5.599c0-.424-1.056-2.09-3.105-3.495" />
                                        </svg>
                            ...
                            <div class="flex justify-end">
                                <button type="submit" :disabled="isDevSubmitting"
                                    ...
                                    {{ isDevSubmitting ? 'Updating...' : 'Update Password' }}
                                </button>
                            </div>
                        </form>
                    </div>
...
const devtabs = [
    { id: 'profile', name: 'Developer Profile Details' },
    { id: 'security', name: 'Security' }
]

const activeTab = ref('profile')
const devActiveTab = ref('profile')
...
const showDevNewPassword = ref(false)
...
const setIsDevSubmitting = (value) => {
    isDevSubmitting.value = value
}
...
const handleDevProfileUpdate = async () => {
    try {
...
const handleDevPasswordChange = async () => {


    if (devPasswordForm.value.newPassword.length < 8) {
        toast.error('Password must be at least 8 characters long')
        return
    }
    ...
    try {
        ...
        await axios.put(`${useRuntimeConfig().public.apiBase}/api/developers/change-dev-password`,
            {

                password: devPasswordForm.value.currentPassword,
                newPassword: devPasswordForm.value.newPassword,
            },
            {
                headers: {
                    Authorization: `Bearer ${token}`
...
const fetchDeveloperDetails = async () => {
    try {
        ...
        const response = await axios.get(`${useRuntimeConfig().public.apiBase}/api/developer-info`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        })

        developerData.value = response.data.developerInfo

        // Populate profile form with current data
        devProfileForm.value = {
            name: response.data.developerInfo.name || '',
            email: response.data.developerInfo.email || '',
        }
    } catch (err) {
        console.error(err)
        toast.error('Failed to fetch user details.')
